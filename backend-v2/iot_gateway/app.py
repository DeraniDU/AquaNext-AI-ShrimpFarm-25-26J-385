...
DO_CRITICAL_LOW  = 3.0   # mg/L — turn aerator ON below this (measured/ML DO)
DO_SAFE_LEVEL    = 4.0   # mg/L — turn aerator OFF above this (hysteresis)
DO_PREEMPT_LOW   = 3.5   # mg/L — turn ON early from temperature prediction
...
def receive_sensor_data():
    ...
    try:
        ...
        # ── Relay State: Use Arduino's physical relay first ──────────────────
        current_do = (
            reading.ml_predictions.get("predicted_do_mg_l")
            if reading.ml_predictions and "predicted_do_mg_l" in reading.ml_predictions
            else reading.do_mg_l
        )
        arduino_relay = data.get("relay_state")  # "ON" or "OFF" string from Arduino
        if arduino_relay and str(arduino_relay).upper() in ("ON", "OFF"):
            # Arduino is in control — mirror its physical state directly
            cmd = str(arduino_relay).upper()
            relay_state["aerator"]       = cmd
            relay_state["trigger_source"]= "arduino_hardware"
            relay_state["do_level"]      = round(current_do, 3) if current_do else None
            if cmd == "ON":
...
def subscribe_newsletter():
    """Handle newsletter subscriptions and send welcome email"""
    data = request.get_json()
    ...
    # Optional: Save to MongoDB here if you create a 'subscribers' collection
    # db.subscribers.insert_one({"email": email, "date": datetime.utcnow()})
    ...
    # If no credentials, simulate success (good for dev)
    if not smtp_user or not smtp_pass:
        logger.info(f"📬 [MOCK EMAIL] Saved subscriber '{email}'. Configure SMTP_USERNAME/SMTP_PASSWORD in .env to send real emails.")
        return jsonify({"status": "success", "message": "Subscribed successfully! (Mocked)"}), 200
        
    try:
        msg = EmailMessage()
        msg['Subject'] = 'Welcome to AquaNext Research Insights!'
        msg['From'] = formataddr(('AquaNext Team', smtp_user))
        msg['To'] = email
