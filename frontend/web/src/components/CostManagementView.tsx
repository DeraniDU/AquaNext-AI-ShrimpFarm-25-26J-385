import type { DashboardApiResponse, BudgetCategorySummary, CostBreakdown, SavingsOpportunity } from '../lib/types'
...
function CostMixBar({ row }: { row: CostBreakdown }) {
	const parts = [
...
function priorityBadgeClass(p: SavingsOpportunity['priority']) {
...
export function CostManagementView({ data, pondFilter, onOpenSettings }: Props) {
	...
	const focusRow: CostBreakdown = pondFilter
		...
		<div className="costManagementPage">
			...
			<div className="costTwoCol">
				<section className="panel costPanel">
					...
					<p className="muted costPanelIntro">Unit rates used for cost and revenue (API / Settings).</p>
				...
				<section className="panel costPanel">
					...
					<p className="muted costPanelIntro">Reference budgets from Settings (sent with each dashboard request).</p>
					{budgetSettings ? (
						<ul className="costAssumptionList">
							<li>
