...
interface IotReading {
  ...
  // Arduino sends relay_state as a plain string "ON"/"OFF";
  // the gateway ML logic sends it as { aerator: 'ON' | 'OFF', ... }
  relay_state?: { aerator: 'ON' | 'OFF'; reason?: string; do_level?: number } | string
}
...
function buildDashboard(r: IotReading, ponds: number): DashboardApiResponse {
  ...
  return {
    ...
    ...(({ extraIot: {
      ...
      alerts_raw: r.alerts,
      // Normalise: if relay_state is a plain string, wrap it into object format
      relay_state: typeof r.relay_state === 'string'
        ? { aerator: (r.relay_state.toUpperCase() === 'ON' ? 'ON' : 'OFF') as 'ON' | 'OFF',
            reason: r.relay_state.toUpperCase() === 'ON' ? 'Temperature exceeded 29.7°C' : undefined }
        : r.relay_state,
      timestamp: r.timestamp,
    } }) as any),
  }
