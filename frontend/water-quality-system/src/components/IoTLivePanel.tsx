...
// ─── EmailJS Config ──────────────────────────────────────────────────────────
const EJS_SERVICE_ID  = 'service_ury2dlf'
const EJS_TEMPLATE_ID = 'template_ne2p6bj'
const EJS_PUBLIC_KEY  = '3Gsdv9gIxDk1knztL'
const ALERT_EMAIL     = 'deranindu@gmail.com'
...
export function IoTLivePanel({ extraIot, error }: { extraIot: ExtraIotFields; error: string | null }) {
  ...
  // Handle relay_state being either an object {aerator:'ON'} OR a plain string 'ON'
  const rawRelayState: unknown = extraIot.relay_state
  const aeratorOn =
    relay?.aerator === 'ON' ||
    (typeof rawRelayState === 'string' && (rawRelayState as string).toUpperCase() === 'ON')
  ...
  // ─── Email alert: fire once when relay flips ON ──────────────────────────
  const prevAeratorRef = useRef<boolean | null>(null)
  const emailSentRef   = useRef<boolean>(false)

  // Helper: sends the alert email
  const sendAeratorEmail = () => {
    const now    = new Date().toLocaleString()
    const temp   = extraIot.temperature != null ? `${extraIot.temperature.toFixed(1)} °C` : 'N/A'
    const reason = relay?.reason ?? 'Temperature exceeded threshold (29.7 °C)'
    console.log('[EmailJS] Sending alert...', { to: ALERT_EMAIL, temp, reason })
    emailjs.send(
      EJS_SERVICE_ID,
      EJS_TEMPLATE_ID,
      {
        to_email   : ALERT_EMAIL,
        to_name    : 'AquaNext Operator',
        subject    : '🚨 AquaNext Alert – Aerator Motor Activated',
        alert_time : now,
        temperature: temp,
        reason     : reason,
        device_id  : extraIot.device_id ?? 'arduino_uno_01',
        message    : `The aerator motor has been automatically turned ON at ${now}. Current temperature: ${temp}. Reason: ${reason}. Please check the pond.`,
      },
      EJS_PUBLIC_KEY,
    )
    .then(() => {
      console.log('[EmailJS] ✅ Alert sent successfully to', ALERT_EMAIL)
      alert('✅ Test email sent! Check your inbox at ' + ALERT_EMAIL)
    })
    .catch((err: unknown) => {
      console.error('[EmailJS] ❌ Failed:', err)
      alert('❌ Email failed. Check browser console for details.')
    })
  }

  useEffect(() => {
    // DEBUG: log every time aeratorOn changes so we can see the value
    console.log(
      `[Relay] aeratorOn=${aeratorOn} | prev=${prevAeratorRef.current} | rawRelayState=`,
      extraIot.relay_state
    )

    const justTurnedOn = aeratorOn && prevAeratorRef.current !== true
    prevAeratorRef.current = aeratorOn
    ...
    if (justTurnedOn && !emailSentRef.current) {
      emailSentRef.current = true
      console.log('[EmailJS] Auto-trigger: aerator just turned ON — sending alert email...')
      // Use emailjs directly here (no alert popup for auto-trigger)
      const now    = new Date().toLocaleString()
      const temp   = extraIot.temperature != null ? `${extraIot.temperature.toFixed(1)} °C` : 'N/A'
      const reason = relay?.reason ?? 'Temperature exceeded threshold (29.7 °C)'
      emailjs.send(
        ...
        EJS_TEMPLATE_ID,
        {
          ...
          reason     : reason,
          device_id  : extraIot.device_id ?? 'arduino_uno_01',
          message    : `Aerator turned ON at ${now}. Temperature: ${temp}. Reason: ${reason}.`,
        },
        EJS_PUBLIC_KEY,
      )
      .then(() => console.log('[EmailJS] ✅ Auto alert sent to', ALERT_EMAIL))
      .catch((err: unknown) => console.error('[EmailJS] ❌ Auto alert failed:', err))
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [aeratorOn, extraIot.relay_state])
  // ─────────────────────────────────────────────────────────────────────────
  ...
  // Device Connection Status Calculation
  let deviceConnected = false
  let timeAgoText = 'Unknown'
  
  if (extraIot.timestamp) {
    // Append 'Z' to make it parse as UTC if it doesn't already have tz info
    const tsStr = extraIot.timestamp.endsWith('Z') ? extraIot.timestamp : extraIot.timestamp + 'Z'
    const lastUpdate = new Date(tsStr)
    const now = new Date()
    const diffSeconds = Math.floor((now.getTime() - lastUpdate.getTime()) / 1000)
    
    // Connected if data received in the last 15 seconds
    deviceConnected = diffSeconds < 15 && diffSeconds >= 0
    
    if (diffSeconds < 60) {
      timeAgoText = 'Just now'
    } else if (diffSeconds < 3600) {
      timeAgoText = `${Math.floor(diffSeconds / 60)} mins ago`
  ...
  return (
    <div style={{ marginBottom: 40, fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div style={{
        ...
        background: deviceConnected ? '#f0fdf4' : '#f3f4f6',
        border: `1px solid ${deviceConnected ? '#bbf7d0' : '#e5e7eb'}`,
        boxShadow: '0 1px 3px rgba(0,0,0,0.05)'
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{
            width: 12, height: 12, borderRadius: '50%',
            background: deviceConnected ? '#22c55e' : '#9ca3af',
            boxShadow: deviceConnected ? '0 0 8px #22c55e' : 'none'
          }} />
          <div>
            <div style={{ fontWeight: 700, color: '#1f2937', fontSize: '0.95rem' }}>
              Water Quality Monitor Device ({extraIot.device_id || 'Unknown'})
            </div>
            <div style={{ fontSize: '0.8rem', color: '#6b7280' }}>
              Status: <span style={{ color: deviceConnected ? '#16a34a' : '#6b7280', fontWeight: 600 }}>
                {deviceConnected ? 'Connected (Live)' : 'Offline'}
              </span>
            </div>
          </div>
        </div>
        <div style={{ textAlign: 'right', fontSize: '0.8rem', color: '#6b7280' }}>
          ...
          <div style={{ fontWeight: 600, color: '#374151' }}>{timeAgoText}</div>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          {/* Test email button — remove after verifying */}
          <button
            id="test-email-btn"
            onClick={sendAeratorEmail}
            style={{
              marginLeft: 8, padding: '4px 12px', borderRadius: 6, border: 'none',
              background: '#1e40af', color: 'white', fontWeight: 700,
